...
export default function SeriesPage() {
  ...
  const loadData = async () => {
    const [{ data: seriesData }, { data: genresData }] = await Promise.all([
      (supabase as any).from('content').select('*').eq('type', 'series').eq('is_published', true).order('created_at', { ascending: false }),
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-8">
      ...
      {filtered.length === 0 ? (
        <p className="py-16 text-center text-dark-400">No series found</p>
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {filtered.map((s) => (
            <ContentCard key={s.id} content={s} inList={myListIds.has(s.id)} onToggleList={toggleList} />
          ))}
        </div>
      )}
    </div>
